eipc = ModuleDeclaration

ModuleDeclaration "module"
  = 'module 'name:ModuleName(newline*)body:ModuleBody {
    return {
      type: "Module",
      name: name.name,
      body: body
...
ValidatorGrammar "validator_grammar"
  = ValidatorAndGrammar / ValidatorOrGrammar
  
ValidatorAndGrammar "validator_and_grammar"
  = MaybeWhiteSpace'AND('newline first:ValidatorStatement rest:(ValidatorStatement)*MaybeWhiteSpace')' {
    return {
      type: 'Condition',
      operation: 'And',
      conditions: [first].concat(rest)
    }
  }
  
ValidatorOrGrammar "validator_or_grammar"
  = MaybeWhiteSpace'OR('newline first:ValidatorStatement rest:(ValidatorStatement)*MaybeWhiteSpace')' {
    return {
      type: 'Condition',
      operation: 'Or',
      conditions: [first].concat(rest)
...
StringOrBoolean "string_or_boolean"
...
Enum "enum"
  = 'enum 'name:IdentifierName' {'newline options:(EnumOption / EnumValuedOption)*'}'newline* {
    return {
      type: 'Enum',
      name: name.name,
      options: options
...
EnumOption "enum_option"
  = MaybeWhiteSpace option:IdentifierName newline {
    return {
      key: option.name,
      value: option.name
    }
...
EnumValuedOption "enum_valued_option"
  = MaybeWhiteSpace name:IdentifierName' = 'value:StringLiteral newline {
    return {
      key: name.name,
      value: value.value
    }
...
StructureBlock "structure_block"
  = '{' newline props:(StructureProperty*) MaybeWhiteSpace'}' {
    return {
...
SubType "subtype"
  = 'subtype ' name:IdentifierName MaybeWhiteSpace'='MaybeWhiteSpace parent:IdentifierName'('newline restrictions:(SubTypeRestriction*)')' newline* {
    return {
      type: 'SubType',
      name: name.name,
      parent: parent.name,
...
SubTypeRestriction "subtype_restriction"
  = MaybeWhiteSpace restrictionName:IdentifierName':' MaybeWhiteSpace value:(StringLiteral / BooleanLiteral / NumberLiteral) newline {
    return {
      name: restrictionName.name,
      value: value
    }
  }

Interface "interface"
  = tags:BlockTagGroup'interface 'name:IdentifierName' {'newline body:InterfaceBody'}'newline* {
    return {
      type: 'Interface',
      tags: tags,
      name: name.name,
...
BlockTagGroup "block_tag_group"
  = tags:(BlockTag newline)* {
    return tags.map(([tag]) => tag)
  }
  
BlockTag "block_tag"
  = MaybeWhiteSpace'['key:IdentifierName'='?value:IdentifierName?']' {
    return {
      type: 'BlockTag',
      key: key.name,
...
ArgumentsSpread "arguments"
  = first_arg:Argument args:(', 'Argument)* {
    return [first_arg].concat(args.map(([comma, arg]) => arg))
  }
...
//SingleLineIgnore = newline / SingleLineComment
//MultiLineIgnore = SingleLineIgnore*
WhiteSpace = ' '+
MaybeWhiteSpace = WhiteSpace?
//SingleLineComment = MaybeWhiteSpace'//'[^\n]+ {
//  return {
//  	type: 'Comment'
//  }
//}
